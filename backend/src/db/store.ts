...
export class V3Store {
  ...
  readTemplate(id: string): TemplateRecord | null {
  ...
  createTemplate(input: { name: string; content: string; tags?: string[] }): TemplateRecord {
  ...
  updateTemplate(id: string, patch: { name?: string; content?: string; tags?: string[] }): TemplateRecord | null {
  ...
  readProfile(id: string): ProfileRecord | null {
    const row = this.db.prepare(`
      SELECT id, name, prepend_text, append_text, model, reasoning_effort, approval_policy, sandbox_mode, service_tier, personality, created_at, updated_at
      FROM profiles
      WHERE id = ?
  ...
  createProfile(input: Omit<ProfileRecord, "id" | "createdAt" | "updatedAt">): ProfileRecord {
    const now = new Date().toISOString();
  ...
  updateProfile(id: string, patch: Partial<Omit<ProfileRecord, "id" | "createdAt" | "updatedAt">>): ProfileRecord | null {
  ...
  recordHistory(input: Omit<HistoryRecord, "id" | "createdAt">): HistoryRecord {
