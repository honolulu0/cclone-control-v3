...
import schemaJson from "../../schemas/codex-config.schema.json";
...
const schemaRoot = schemaJson as JsonSchemaNode;

export async function readCodexSettings(hostClient: HostClient): Promise<CodexSettingsSnapshot> {
  ...
  const fields = Object.entries(schemaRoot.properties || {})
    .map(([configKey, rawNode]) => buildFieldFromSchema(configKey, rawNode as JsonSchemaNode, configRoot, origins, saveTarget))
...
function compareGroup(left: CodexSettingField["group"], right: CodexSettingField["group"]): number {
  const order: CodexSettingField["group"][] = ["instructions", "model", "runtime", "behavior", "integration", "advanced"];
  return order.indexOf(left) - order.indexOf(right);
}

export async function readCodexSettingsSchema(hostClient: HostClient): Promise<CodexSettingsSchema> {
...
export async function writeCodexSettings(hostClient: HostClient, patch: Record<string, unknown>): Promise<CodexSettingsSnapshot> {
  const schema = await readCodexSettingsSchema(hostClient);
  const schemaByKey = new Map(schema.fields.map((field) => [field.key, field]));
  const edits: Array<{ keyPath: string; value: unknown; mergeStrategy: "upsert" }> = [];

  for (const [key, value] of Object.entries(patch)) {
    const field = schemaByKey.get(key);
    if (!field) {
      throw createHttpError(400, `Unsupported Codex setting: ${key}`);
    }
    if (!field.editable) {
      throw createHttpError(400, `Codex setting is read-only: ${key}`);
    }
...
function buildFieldFromSchema(
  ...
  saveTarget: Pick<CodexSettingField, "saveTargetLabel" | "saveTargetPath" | "saveTargetDetail">,
): CodexSettingField {
...
function resolveSchemaRef(ref: string): JsonSchemaNode {
...
function inferFieldType(configKey: string, node: JsonSchemaNode, value: unknown): CodexSettingField["type"] {
  ...
  if (node.type === "array" || node.type === "object" || node.properties || node.additionalProperties) return "json";
  if (configKey.includes("instructions") || configKey.includes("prompt")) return "textarea";
  return "text";
}

function extractOptions(configKey: string, node: JsonSchemaNode): Array<{ label: string; value: string }> {
  if (configKey === "approval_policy") {
    return [
  ...
  if (Array.isArray(node.enum)) {
    return node.enum.map((value: unknown) => ({ label: String(value), value: String(value) }));
  }
...
function hasEnum(node: JsonSchemaNode): boolean {
  if (Array.isArray(node.enum) && node.enum.length > 0) {
    return true;
  }
  return ["oneOf", "anyOf"].some((key) =>
    Array.isArray(node[key]) &&
    node[key].every((entry: JsonSchemaNode) => {
...
function describeField(configKey: string, baseDescription: string): string {
...
function usageForField(
  configKey: string,
  type: CodexSettingField["type"],
  options: Array<{ label: string; value: string }>,
): string {
  if (configKey === "instructions") {
...
function inferGroup(configKey: string): CodexSettingField["group"] {
  if (configKey.includes("instruction") || configKey.includes("prompt")) return "instructions";
  if (configKey === "model" || configKey.startsWith("model_") || configKey === "review_model" || configKey === "service_tier") return "model";
  if (configKey.includes("approval") || configKey.includes("sandbox") || configKey === "permissions" || configKey === "default_permissions") return "runtime";
  if (configKey === "personality" || configKey.includes("history") || configKey.includes("memories")) return "behavior";
  if (configKey.includes("mcp") || configKey.includes("plugin") || configKey.includes("tool") || configKey.includes("app") || configKey.includes("market") || configKey.includes("project")) return "integration";
  return "advanced";
}

function inferSource(origins: Record<string, any>, configKey: string): Pick<CodexSettingField, "source" | "sourceLabel" | "sourcePath" | "sourceDetail"> {
...
function extractConfigRoot(payload: any): Record<string, unknown> {
...
function extractOrigins(payload: any): Record<string, any> {
  return payload && typeof payload.origins === "object" && payload.origins
    ? payload.origins as Record<string, any>
    : {};
}

function inferSaveTarget(payload: any): Pick<CodexSettingField, "saveTargetLabel" | "saveTargetPath" | "saveTargetDetail"> {
  const userConfigPath = extractUserConfigPath(payload);
  const saveTargetLabel = userConfigPath ? path.basename(userConfigPath) : "config.toml";
  return {
    saveTargetLabel,
...
function inferCliCommand(configKey: string): Pick<CodexSettingField, "cliCommand" | "cliCommandDetail"> {
  const dedicated: Record<string, { command: string; detail: string }> = {
    model: {
...
function extractUserConfigPath(payload: any): string | null {
...
function omit<T extends Record<string, unknown>>(value: T, keys: string[]): Partial<T> {
  const next = { ...value };
